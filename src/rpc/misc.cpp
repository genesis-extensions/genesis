...
{
    if (type == 2) {
        address = CGenesisAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CGenesisAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CGenesisAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        if (!addressValues.isArray()) {
         ...
         for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
             CGenesisAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
