...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
    ...
    {
        LogPrintf("CheckProofOfWork failed the first check: \n");
        if (fNegative)
        {
            LogPrintf("fNegative is true \n");
        }
        else if (bnTarget == 0)
        {
            LogPrintf("bnTarget is zero \n");
        }
        else if (fOverflow)
        {
            LogPrintf("fOverflow is true \n");
        }
        else if (bnTarget > UintToArith256(params.powLimit))
        {
            LogPrintf("bnTarget (%d) is greater than the minimum difficulty (%d) \n", bnTarget.GetCompact(), UintToArith256(params.powLimit).GetCompact());
        }
        return false;
    }
    ...
    {
        LogPrintf("CheckProofOfWork failed the second check: the hash (%d) is greater than the target (%d) \n", UintToArith256(hash).GetCompact(), bnTarget.GetCompact());
        return false;
    }

    //LogPrintf("CheckProofOfWork success! \n");
    return true;
}
