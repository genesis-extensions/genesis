...
Below are some notes on how to build SafeCa$h for Windows.

The options known to work for building SafeCa$h on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the SafeCa$h Windows release binaries.
...
Note that for WSL the SafeCa$h source path MUST be somewhere in the default mount file system, for
...
causes two of the safecash executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the safecash code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the safecash source code).
